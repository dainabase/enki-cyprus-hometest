...
const propertyTypes = ['Appartement', 'Maison', 'Villa'];
...
// Coordonnées approximatives des villes chypriotes
...
function generateRandomCoordinates(city) {
  ...
  // Ajouter une variation aléatoire de ±0.02 degrés
  return {
...
function generatePhotos(type, index) {
  const count = Math.floor(Math.random() * 3) + 3; // 3-5 photos
  const photos = [];
  const typeQuery = type.toLowerCase() === 'appartement' ? 'apartment' : type.toLowerCase();
  
  for (let i = 0; i < count; i++) {
    photos.push(`https://source.unsplash.com/random/800x600/?cyprus+${typeQuery}&sig=${index}${i}`);
  }
  return photos;
}

function generatePlans(index) {
  const count = Math.floor(Math.random() * 3) + 1; // 1-3 plans
  const plans = [];
  
  for (let i = 0; i < count; i++) {
    plans.push(`https://placehold.co/800x600/png?text=Floor+Plan+${i + 1}&font=roboto&seed=${index}${i}`);
  }
  return plans;
}

function generateVirtualTour(index) {
  const tourIds = ['abcdef123', 'ghijkl456', 'mnopqr789', 'stuvwx012', 'yzabcd345'];
  const randomId = tourIds[index % tourIds.length] + index;
  return `https://my.matterport.com/show/?m=${randomId}`;
}

function generateDescription(type, city, bedrooms, size) {
  const descriptions = {
    'Appartement': [
...
async function generateProjects() {
  ...
  // Distribution : 30 appartements (index 1-30), 20 maisons/villas (index 31-49)
  for (let i = 1; i < 50; i++) {
    ...
    const size = type === 'Appartement' ? 
      Math.floor(Math.random() * 80) + 40 : // 40-120m² pour appartements
      Math.floor(Math.random() * 150) + 100; // 100-250m² pour maisons/villas
    
    const description = generateDescription(type, city, bedrooms, size);
    const featuresCount = Math.floor(Math.random() * 6) + 5; // 5-10 features
    ...
    projects.push({
      ...
      features,
      photos: generatePhotos(type, i),
      plans: generatePlans(i),
      virtual_tour: generateVirtualTour(i)
    });
  }
