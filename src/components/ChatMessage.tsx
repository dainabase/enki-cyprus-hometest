          ...
          <>
            ...
            {message.properties && message.properties.length > 0 && (
              <div className="mt-4 space-y-3">
                <h4 className="font-semibold text-primary text-sm">Propriétés recommandées :</h4>
                <div className="grid gap-3 max-h-96 overflow-y-auto">
                  {message.properties.slice(0, 3).map(property => (
                    <div key={property.id} className="bg-white rounded-lg p-3 border border-gray-200/50 shadow-sm">
                      <h5 className="font-semibold text-sm text-gray-800 mb-1">{property.title}</h5>
                      <p className="text-xs text-gray-600 mb-2">{typeof property.location === 'string' ? property.location : (property.location as any)?.city || 'Chypre'}</p>
                ...
                {message.properties.length > 3 && (
                  <p className="text-xs text-gray-500 italic">
                    +{message.properties.length - 3} autres propriétés disponibles
                  </p>
                )}
              </div>
            )}
            ...
            {message.fiscalOptimization && (
              <div className="mt-4 p-3 bg-blue-50/80 rounded-lg border border-blue-200/50">
                ...
                <p className="text-sm text-blue-700 mb-3">{message.fiscalOptimization.preview}</p>
