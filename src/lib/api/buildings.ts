...
export const createBuildingGlobal = async (buildingData: BuildingFormData): Promise<Building> => {
  ...
  const { data, error } = await supabase
    ...
    .insert([{
      ...
      energy_efficiency_class: buildingData.energy_efficiency_class || '',
...
export const updateBuilding = async (id: string, buildingData: Partial<BuildingFormData>): Promise<Building> => {
  ...
  const updateData = {
    ...
    ...(buildingData.energy_efficiency_class !== undefined && { energy_efficiency_class: buildingData.energy_efficiency_class }),
