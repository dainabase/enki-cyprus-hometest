...
export class AgentSEO {
  ...
  async genererContenuSEO(donnees: DonneesProjet, langue: string = 'fr'): Promise<ContenuSEO> {
    ...
    // Prompt ULTRA-SPÉCIFIQUE pour Chypre
    const promptSysteme = `Tu es l'expert SEO immobilier #1 de Chypre depuis 10 ans.
      
      TU CONNAIS PARFAITEMENT:
      - Programme Golden Visa (seuil €300k, process 2 mois)
      - Marché par zone: ${Object.entries(this.MARCHE_CHYPRE.zones).map(([z, d]) => `${z}: ${d.points_forts}`).join(', ')}
      - Psychologie acheteurs: Russes (sécurité), Chinois (éducation), Européens (lifestyle)
      - Mots-clés performants 2024-2025
      - Concurrence locale et internationale
      
      RÈGLES SEO STRICTES:
      - Titre: EXACTEMENT 55-60 caractères (Google limite)
      - Description: EXACTEMENT 155-160 caractères (snippet optimal)
      - ${estGoldenVisa ? 'TOUJOURS mentionner Golden Visa en premier' : 'Focus sur avantages uniques'}
      - Utiliser année ${anneeActuelle} pour fraîcheur SEO
      - Power words: Exclusive, Premium, Luxury, Investment, Secure
      - Inclure prix range si pertinent
      - Format JSON parfait uniquement
      
      CONTEXTE MARCHÉ ${anneeActuelle}:
      - Demande Golden Visa forte (post-guerre Ukraine)
      - Investisseurs tech en croissance
      - Marché immobilier ${zone.croissance_annuelle} en ${donnees.zone}
      - Prix moyen zone: €${zone.prix_moyen_m2}/m²`;
    
    const promptUtilisateur = `
      PROJET À OPTIMISER:
      📍 Nom: ${donnees.nom_projet}
      📍 Zone: ${donnees.zone} (${zone.points_forts})
      ...
      ${estGoldenVisa ? '🏆 GOLDEN VISA ELIGIBLE - PRIORITÉ ABSOLUE' : ''}
      
      GÉNÈRE CONTENU SEO OPTIMAL:
      
      1. titre_meta: [Nom Projet] - [Type] ${donnees.zone} | ${estGoldenVisa ? 'Golden Visa ' : ''}Cyprus ${anneeActuelle}
         - Exactement 55-60 caractères
         - Inclure prix si <300k€ OU >1M€
      
      2. description_meta: Description vendeuse avec prix, USP unique, ${estGoldenVisa ? 'Golden Visa, ' : ''}CTA fort
         - Exactement 155-160 caractères
         - Call-to-action irrésistible
      
      3. mots_cles: Array de 8-12 mots-clés (mix ${zone.mots_cles.join(', ')} + spécifiques projet)
         - Longue traîne incluse
         - Commercial intent
      
      4. points_marketing: 5 points bullet vendeurs
         - ${estGoldenVisa ? 'Point 1 OBLIGATOIRE: Golden Visa eligibility' : 'USPs différenciants'}
         - Bénéfices clients finaux
      
      5. audience_cible: Description précise du buyer persona "${segment}"
      
      6. slug_url: format-kebab-case-avec-zone-et-prix-range
    ...
    try {
      ...
      const reponse = await this.appelerOpenAI(promptSysteme, promptUtilisateur);
  ...
  private async appelerOpenAI(system: string, user: string): Promise<any> {
