-- =====================================================
-- AUDIT ET OPTIMISATION BASE DE DONNÉES ENKI REALITY
-- Date: 2025-01-13
...
-- Contraintes de validation Cyprus
ALTER TABLE properties ADD CONSTRAINT IF NOT EXISTS check_energy_rating 
    CHECK (energy_certificate_rating IN ('A', 'B', 'C', 'D', 'E', 'F', 'G') OR energy_certificate_rating IS NULL);

ALTER TABLE properties ADD CONSTRAINT IF NOT EXISTS check_vat_rate_cyprus 
    CHECK (vat_rate IN (5.00, 19.00) OR vat_rate IS NULL);

-- Contraintes prix logiques
ALTER TABLE properties ADD CONSTRAINT IF NOT EXISTS check_price_positive 
    CHECK (price > 0);

ALTER TABLE properties ADD CONSTRAINT IF NOT EXISTS check_size_positive 
    CHECK (size_m2 > 0);

-- 4. MISE À JOUR DES DONNÉES EXISTANTES

-- Mise à jour des VAT rates manquants
UPDATE properties 
SET vat_rate = CASE 
