-- ÉTAPE 1/3 : MIGRATION COMPLÈTE BASE DE DONNÉES - NOUVELLE ARCHITECTURE CASCADE (CORRECTION)
-- =====================================================================================

-- 1. BACKUP COMPLET AVANT MIGRATION (avec les bonnes tables)
CREATE TABLE backup_complete_20250120 AS 
SELECT 'developers' as table_name, row_to_json(d.*) as data FROM developers d
UNION ALL
SELECT 'projects_clean', row_to_json(p.*) FROM projects_clean p
UNION ALL  
SELECT 'buildings_enhanced', row_to_json(b.*) FROM buildings_enhanced b
UNION ALL
SELECT 'properties_final', row_to_json(p.*) FROM properties_final p;

-- 2. SUPPRESSION TABLES ACTUELLES (dans l'ordre inverse des dépendances)
DROP TABLE IF EXISTS properties_final CASCADE;
DROP TABLE IF EXISTS buildings_enhanced CASCADE;
