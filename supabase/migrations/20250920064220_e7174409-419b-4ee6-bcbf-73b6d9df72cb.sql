-- ÉTAPE 2/3 : IMPORT DES DONNÉES AVEC CONTRAINTES CORRIGÉES
-- =======================================================

-- 1. IMPORT DEVELOPERS (21 entrées) - FIXED CONSTRAINT VIOLATIONS
...
SELECT 
  ...
  -- Fix rating_score: constrain to 0-5 range
  CASE 
    WHEN rating_score IS NULL THEN NULL
    WHEN rating_score > 5 THEN 5.0
    WHEN rating_score < 0 THEN 0.0
    ELSE rating_score 
  END as rating_score,
  payment_terms, 
...
SELECT 
  p.id, p.developer_id, p.title, p.subtitle, p.description, p.detailed_description,
  COALESCE(p.city, 'Limassol') as city, p.region, p.neighborhood, p.full_address, p.cyprus_zone,
  p.gps_latitude, p.gps_longitude,
  p.proximity_sea_km, p.proximity_airport_km, p.proximity_city_center_km, p.proximity_highway_km,
  p.land_area_m2, p.built_area_m2,
...
INSERT INTO buildings (
  ...
  elevator_count, has_generator, has_security_system, has_cctv, 
  has_concierge, has_solar_panels, has_pool, has_gym, has_spa,
...
INSERT INTO properties (
  ...
  property_code, unit_number, property_type, property_sub_type, property_status,
  floor_number, orientation, internal_area, covered_verandas, balcony_area,
  private_garden_area, roof_garden_area, bedrooms_count, bathrooms_count, wc_count,
  price_excluding_vat, parking_spaces, parking_type_unit,
  has_sea_view, has_mountain_view, has_city_view,
  is_available, is_furnished, created_at, updated_at
)
SELECT
  ...
  p.floor_number, p.orientation,
  COALESCE(p.internal_area, 100),
  p.covered_verandas, p.balcony_area, p.private_garden_area, p.roof_garden_area,
...
-- 5. Si aucune property dans le backup, créer 3 properties test
INSERT INTO properties (
...
SELECT 
  p.id as project_id, b.id as building_id, p.developer_id,
  p.id::text || '-PH0' || row_number() OVER () as property_code,
  'PH0' || row_number() OVER () as unit_number,
  'penthouse' as property_type, 'available' as property_status,
  5 as floor_number, 150 as internal_area,
  3 as bedrooms_count, 2 as bathrooms_count,
...
-- 6. VÉRIFICATIONS POST-IMPORT ET RAPPORT FINAL
SELECT 
...
-- Vérifier les cascades de données
SELECT 
...
-- Vérifier quelques exemples de prix Golden Visa
SELECT 
  property_code,
  price_excluding_vat,
  vat_rate,
