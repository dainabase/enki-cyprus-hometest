...
-- 1. IMPORT DEVELOPERS avec rating_score normalisé (0-5)
...
SELECT 
  ...
  -- Normalize rating_score from 0-10 scale to 0-5 scale
  CASE 
    WHEN rating_score IS NULL THEN NULL
    WHEN rating_score > 10 THEN 5  -- Cap at 5
    WHEN rating_score > 5 THEN rating_score / 2  -- Convert 0-10 to 0-5
    ELSE rating_score 
  END as rating_score,
  payment_terms, 
...
SELECT 
  ...
  p.title_deed_status, p.planning_permit_number, p.building_permit_number,
  CASE 
...
-- 5. Fallback properties si aucune property importée
INSERT INTO properties (
...
SELECT 
  ...
  (SELECT COUNT(*) FROM properties WHERE golden_visa_eligible = true) as golden_visa_count;