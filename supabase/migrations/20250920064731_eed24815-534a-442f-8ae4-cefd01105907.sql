-- ÉTAPE 2/3 : IMPORT FINAL AVEC COLONNES EXISTANTES UNIQUEMENT
-- ==============================================================

-- 1. IMPORT DEVELOPERS (corrigé)
...
SELECT 
  ...
  CASE 
    ...
    WHEN rating_score > 10 THEN 5  
    WHEN rating_score > 5 THEN rating_score / 2  
    ELSE rating_score 
  END as rating_score,
  payment_terms, 
...
-- 2. IMPORT PROJECTS (colonnes existantes uniquement)
...
SELECT 
  ...
  p.region, p.neighborhood, p.full_address, p.cyprus_zone,
  ...
  p.roi_estimate_percent, p.rental_yield_percent,
  CASE 
...
SELECT
  ...
  b.parking_type,
  b.expected_completion, b.actual_completion,
...
SELECT
  ...
  p.covered_verandas, p.balcony_area,
  p.private_garden_area, p.roof_garden_area,
...
-- 5. CRÉER PROPERTIES SI AUCUNE IMPORTÉE
INSERT INTO properties (
...
-- RAPPORT ÉTAPE 2/3 - IMPORT DES DONNÉES
-- =======================================

SELECT 
  '🎯 ÉTAPE 2/3 TERMINÉE - IMPORT DES DONNÉES' as titre,
  '======================================' as separateur;

SELECT 
  'RÉSULTATS IMPORT' as info,
  (SELECT COUNT(*) FROM developers) as developers_count,
  (SELECT COUNT(*) FROM projects) as projects_count,
  (SELECT COUNT(*) FROM buildings) as buildings_count,
  (SELECT COUNT(*) FROM properties) as properties_count;

-- Vérifications Golden Visa
SELECT 
  'GOLDEN VISA CHECK' as type,
  COUNT(*) as total_properties,
  COUNT(CASE WHEN golden_visa_eligible = true THEN 1 END) as golden_visa_count,
  COUNT(CASE WHEN price_including_vat >= 300000 THEN 1 END) as price_over_300k
FROM properties;

-- Vérifications cascade
SELECT 
  'CASCADE VERIFICATION' as check_type,
